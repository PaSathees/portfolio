...
| ------- | ----------- | ------------ | ---- | ---- | ------------ |
| [Vehicle Tyre Quality Checker 🏍️🚗🛞🔧🧑‍🔧](PYTORCH/Tyre_quality_checker_PyTorch.ipynb) | Vehicle tyres become obsolute as they age and rubber becomes hard. This causes minute cracks in the surface. I am attempting to create a predictor to identify whether a tyre is defective or good from images of the tyres. Deployed it to Hugging Face Spaces | HuggingFace, Gradio, PyTorch, TorchVision, Python, MLFlow, Paperspace, Kaggle | [GitHub Location](PYTORCH/Tyre_quality_checker_PyTorch.ipynb) | [Demo](https://huggingface.co/spaces/PaSathees/Vehicle_Tyre_Quality_Checker) | - Created a predictor to identify whether a tyre is defective or good from images of the tyres by fine-tuning EfficientNet_V2_L pre-trained model on a [Kaggle dataset](https://www.kaggle.com/datasets/warcoder/tyre-quality-classification). Deployed Vehicle Tyre Quality Checker App made with Gradio to Production using HuggingFace Spaces. Used MLFlow to track experimentation and identifying best approach |
